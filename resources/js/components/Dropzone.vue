<template>
    <div ref="container" id="picture-input" class="picture-input mb-3">
        <div v-if="error" role="alert" class="alert alert-danger">
            <div>{{ $t(errorAlert) }}</div>
        </div>
    ...
    methods: {
        updateStrings() {
            for (let s in this.customStrings) {
                if (
                    s in this.strings &&
                    typeof this.customStrings[s] === "string"
                ) {
        ...
        preloadImage(source, options) {
            ...
            fetch(source, {
                ...
                .then(imageBlob => {
                    let e = { target: { files: [] } };
                    const fileName =
                        options.fileName || source.split("/").slice(-1)[0];
                    let mediaType =
                        options.mediaType ||
                        "image/" +
                            (options.fileType ||
